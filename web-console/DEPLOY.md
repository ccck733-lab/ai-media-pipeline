...
控制台成片页已内置「一键发布到平台」卡片（抖音 / 小红书）。发布依赖开放平台 OAuth，需先填凭证。

### 通用：回调地址（两个平台都要填）
在开放平台应用的「授权回调 / Redirect URI」里登记：
```
https://loveshop.us.ci/api/distribution/oauth/callback
```
本地调试可用 `http://localhost:8000/api/distribution/oauth/callback`（部分平台要求已备案 https 域名，正式用 loveshop.us.ci）。

### 抖音（可真·一键发布）
1. 打开 [抖音开放平台](https://open.douyin.com) → 创建**网站应用** → 拿到 `client_key` / `client_secret`。
2. 「接口权限」申请 **视频发布及管理**（审核通过的应用默认开通该权限；scope 用 `video.create,video.publish`）。
3. 把凭证写入 `config/platforms.json` 的 `douyin` 段（或用环境变量 `DOUYIN_CLIENT_KEY` / `DOUYIN_CLIENT_SECRET`）：
   ```json
   { "douyin": { "client_key": "xxx", "client_secret": "yyy" } }
   ```
4. 控制台成片页点「连接账号」→ 抖音扫码授权 → 回控制台点「一键发布」即发到该授权号。access_token 2h 过期，refresh_token 30d，后端自动续期。

### 小红书（公开发布 API 未开放，走发布助手）
- 公开开放平台**没有对普通开发者开放笔记发布 API**（仅白名单 MCN/品牌方，需签保密协议+内容安全网关）。
- 先在 `config/platforms.json` 的 `xiaohongshu` 段填 AppKey/AppSecret（环境变量 `XHS_CLIENT_ID` / `XHS_CLIENT_SECRET`），可点「连接账号」完成 OAuth（仅拿到 token）。
- 点「一键发布」：若你是**白名单合作方且已获批 `note_publish` 权限**，会真实发布；否则接口返回 403，前端会提示改用「复制文案 + 下载」，到 [小红书创作者中心](https://creator.xiaohongshu.com) 手动发布。
- 不实现逆向 x-s 签名发布（违反平台条款且极易失效）。

### 生效
改完 `config/platforms.json` 后**重启后端**（`launchctl load` 或直接 `./run-local-api.sh`）即可，无需重新部署前端。令牌存于 `workspace/.tokens/`（已 gitignore）。
